...
public final class PanView: UIView {
  ...
  public var state: PresentationState = .hidden
...
public extension PanView {
  ...
  func transition(to state: PresentationState, animated: Bool = true) {
    ...
    switch state {
    case .short:
      snap(toYPosition: shortFormYPosition, animated: animated) { [weak self] in
        self?.presentableViewController.didTransition(to: state)
      }
    case .long:
      snap(toYPosition: longFormYPosition, animated: animated) { [weak self] in
        self?.presentableViewController.didTransition(to: state)
      }
    case .hidden:
      snap(toYPosition: hiddenFormYPosition, animated: animated) { [weak self] in
        self?.presentableViewController.didTransition(to: state)
      }
    }
  }
  ...
  @objc func didPanOnPresentedView(_ recognizer: UIPanGestureRecognizer) {
    ...
    case .began, .changed:
      ...
      if panelView.frame.origin.y == anchoredYPosition && extendsPanScrolling {
        ...
        presentableViewController.didTransition(to: .long)
      }
    default:
      /**
